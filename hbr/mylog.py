...
def spend_time(func):
    '''
    This function takes another function func as an argument and returns a decorated version of it (wrappedFunc).
    '''
    @functools.wraps(func) # This decorator ensures that wrappedFunc retains the metadata (like name, docstring) of the original function func.
    def wrappedFunc(*args, **kargs):
        '''
        This is the actual wrapper function that replaces func.
        '''
        starttime = time.time()
        try:
            print("\nCalling: %s" % func.__name__) #Prints a message indicating which function is being called.
            return func(*args, **kargs) # Calls the original function func with its arguments and returns its result.
        finally:
            endtime = time.time()
            print("spend time: %.4fs, %.2fm" % (endtime - starttime, (endtime - starttime)/60)) # Prints the total time spent in seconds (%.4fs) and in minutes (%.2fm).
    return wrappedFunc #Returns the decorated version of func, which measures and prints the execution time whenever func is called.
